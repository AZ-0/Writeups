...
Indeed $(p)\cap\mathbb Z$ must be a prime ideal of $\mathbb Z$ containing $n_p$, but $(n_p)$ is also prime thus maximal because $\mathbb Z$ is principal.
...
In the latter case we would have $p\mathbb Z[i]=n_p\mathbb Z[i]$, which would imply that $n_p$ remains prime in $\mathbb Z[i]$ on account of generating a prime ideal.
This is obviously false since $n_p=p\bar p$.

Now chaining this together with the chinese remainder theorem gives $$\mathbb Z[i]/n\cong\mathbb Z[i]/p\times\mathbb Z[i]/q\cong \mathbb Z/n_p\times \mathbb Z/n_q\cong \mathbb Z/n_n.$$
...
This requires us to find $j$.
To perform that, notice that if $u = u_x+iu_y$ is a gaussian integer then $\mathfrak Im\ un = u_yn_x + u_xn_y$.
Since $\mathrm{gcd}(n_x,n_y)=1$ we can find some $u$ such that $\mathfrak Im\ un=1$.
In particular, if $a=a_x+ia_y$ is any gaussian integer then $\mathfrak Im(a-a_yun)=0$ so that $f(a)=a-a_yun\in\mathbb Z/n_n\mathbb Z$.
```py
def modint(a, n):
...
Given that the mathematical approach has proven quite fruitful to understand what's happening so far, I'd expect there to be a beautiful algebraic attack on grabin that exploits the structure of $\mathbb Z[i]$.
...
They say that for non-Blum primes it "reduces to the classical case" (most likely because of the above isomorphism) without further addo.
...
On the other hand, there have been a great many CTF where a single algebraic relation revealed enough about $n_n$ to factor it.
Perhaps we are in the presence of such a case?
...
Woops.
Looking through `challenge.py` again reveals that the primes are far from honest:
```py
re = randrange(2 ** l)
im = randrange(2 ** l)
p = G(re, im)
...
Now $a$ is well approximated by $\sqrt{\frac{X+Y}2}$ and $b$ by $\sqrt{\frac{X-Y}2}$, as we theoretically miss at most $56$ bits.
```hs
nx = ZZ(n.real())
ny = ZZ(n.imag())
nn = ZZ(n.norm())
...
Since $f(p)$ and $f(q)$ are both non-zero, this means that $p_x + jp_y$ is a zero-divisor, ie has non-trivial gcd with $n_n$: it's a multiple of $n_p$.
...
But this is tweaking hell and I waste several more hours finding the right parameters with three different implementations of Coppersmith, until Defund's `small_roots` works on my locally generated $n$ with bounds $2^{37}$, $m=1$, and $d=5$!
It even works near instantly on everything I generate using the provided challenge file!
So far, so good.
